...
export async function adminAccess(req,res,next){
  try{
    const token=cookies(req.headers.cookie||'')[adminCookieName],session=verifyAdminSession(token);
    ...
    req.adminUser={_id:user._id,name:user.name,username:user.username,email:user.email||'',role:user.role,mustChangePassword:Boolean(user.mustChangePassword)};next();
